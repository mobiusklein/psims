...
def test_writer():
    buffer = io.BytesIO()
    with MzMLbWriter(buffer, close=False) as f:
        ...
        f.software_list([
            f.Software(version="0.0.0", id='psims', params=[
                       'custom unreleased software tool', 'python-psims'])
        ])
        f.instrument_configuration_list([
            ...
            f.InstrumentConfiguration(id=2, component_list=f.ComponentList([
                f.Source(params=['electrospray ionization'], order=1),
                f.Analyzer(
                    params=['radial ejection linear ion trap'], order=2),
                f.Detector(params=['inductive detector'], order=3)
            ]))
        ])
    ...
    assert handle['mzML']
    # The third spectrum is written with a different encoding map, the default.
    assert len(handle['spectrum_MS_1000516_float64']) == len(intensity_array) * 2
    assert len(handle['mzML_spectrumIndex']) == 4
    assert len(bytearray(handle['mzML_spectrumIndex_idRef']).split(b'\x00')) == 4
    assert np.allclose(handle['spectrum_MS_1000514_float64'], mz_array * 3)
